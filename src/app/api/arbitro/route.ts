...
export const POST = async (req: Request) => {
    ...
    const arbitro = await prisma.$queryRaw`INSERT INTO Arbitro (dni_arbitro_fk, nivel_experiencia, tiene_certificacion) VALUES (${dni_arbitro_fk}, ${nivel_experiencia}, ${tiene_certificacion})`;
...
export const PUT = async (req: Request) => {
    try {
        ...
        const arbitro = await prisma.$queryRaw`UPDATE Arbitro SET nivel_experiencia = ${nivel_experiencia}, tiene_certificacion = ${tiene_certificacion} WHERE dni_arbitro_fk = ${dni_arbitro_fk}`;
        return NextResponse.json(arbitro, { status: 201 });
    } catch (e) {
...
export const DELETE = async (req: Request) => {
    try {
        ...
        const arbitro = await prisma.$queryRaw`DELETE FROM Arbitro WHERE dni_arbitro_fk = ${dni_arbitro_fk}`;
        return NextResponse.json(arbitro, { status: 201 });
    } catch (e) {
